...
- [TUNING_GUIDE.md](E:\AURIX-v1.10.28-workspace\LIUHE_bicycle1\TUNING_GUIDE.md)

该文档单独整理了全项目调参变量、调参顺序、原理解释、现象排障和各科目实车调参方法。
...
当前工程已将 `HOME` 页的 `Active` 和舵机控制模式自动绑定：

- `Active=0` → `SERVO_CTRL_MODE_SIMPLE_PD`
- `Active=1` → 科目1 + `SERVO_CTRL_MODE_CASCADE`
- `Active=2` → 科目2 + `SERVO_CTRL_MODE_LOW_SPEED`
- `Active=3` → 科目3 + `SERVO_CTRL_MODE_CASCADE`

切换入口只有一个：

- IPS `HOME` 页修改 `Active`

因此比赛前不需要再手动单独切 `Ctrl`，也不再依赖 `KEY2` 双击切科目。

当前工程已经统一成“显式启动才放行电机”的启停框架：

- 所有模式上电默认都关闭电机输出，即使 `target_motor_rpm` 预设为非 0 也不会直接起跑
- 切换 `Active` 只改变当前科目和 `Ctrl` 对应关系，不会直接放行电机输出
- `KEY3` 长按是唯一的运行入口：`Active=0` 时用于手动测试启停，`Active=1/2/3` 时用于对应科目启停
- 停止或任务正常完成后都会重新关闭电机输出，下一次必须再次 `KEY3` 长按启动
- `Active=0` 停止时不会清掉当前设定的目标转速，便于反复测试同一转速
...
**波形 CH1 说明：**
- CH1 发送的是 `roll_ctrl_angle`，即“已扣除机械零位补偿后的控制用横滚角”。
- 它不是 `roll_kalman` 原始横滚角；调平衡参数、看直立误差时应以 CH1 为准。
...
| `KEY3` | 短按 | IPS 切换到下一页 |
| `KEY3` | 双击 | IPS 切换到上一页 |
| `KEY3` | 长按 | 统一启停入口：`Active=0` 时手动测试启停；`Active=1/2/3` 时按当前科目启动/停止 |
| `KEY4` | 短按 | IPS 选中下一项参数 |
| `KEY4` | 双击 | IPS 选中上一项参数 |
...
- `Active` 现在只在 `HOME` 页按普通参数方式调节，范围 `0~3`；`Ctrl` 由 `Active` 自动同步，不再单独手动切换。
- 所有模式上电默认都不会直接驱动电机，必须 `KEY3` 长按后才真正放行输出。
- `KEY3/KEY4` 的长按功能优先级高于普通页面操作，实际比赛时主要用来启动/停止和勘线。
- Flash 持久化当前以 `KEY1 双击` 为准，README 其余章节若涉及保存动作，也应以此为准。
- 若当前活跃科目正在运行，`Active` 切换会被拦截；只有“未运行”时才允许切换。
- 对科目1来说，`SUBJ1_STATE_GO / GO_BRAKE / UTURN / RETURN` 算运行中；`SUBJ1_STATE_IDLE` 和 `SUBJ1_STATE_DONE` 都算已结束、不在运行。
- 这意味着科目1自然跑完进入 `SUBJ1_STATE_DONE` 后，不需要再额外按一次 `KEY3` 才算结束；此时已经可以切换 `Active`，也可以直接重新勘线。
...
4. 当前版本已修复“回程覆盖工作路点”问题：同一上电周期内再次 `KEY3` 启动时，会先自动恢复原始勘线路线，无需重启或重新勘线
...
当前代码以 [`subject1_app.c`](/E:/AURIX-v1.10.28-workspace/LIUHE_bicycle1/code/app/subject1_app.c) 和 [`key_app.c`](/E:/AURIX-v1.10.28-workspace/LIUHE_bicycle1/code/app/key_app.c) 为准：

- 下面四个状态算“运行中”：`SUBJ1_STATE_GO`、`SUBJ1_STATE_GO_BRAKE`、`SUBJ1_STATE_UTURN`、`SUBJ1_STATE_RETURN`
- 下面两个状态算“未运行”：`SUBJ1_STATE_IDLE`、`SUBJ1_STATE_DONE`

对应操作含义：

- 运行中如果再 `KEY3` 长按，会执行紧急停止，状态回到 `IDLE`
- 如果车辆自己完整跑完一趟，状态会进入 `SUBJ1_STATE_DONE`
- 进入 `DONE` 后，系统已经认为科目1结束了，不需要你再按一次 `KEY3` 才“退出运行”
- 在 `DONE` 或 `IDLE` 状态下，都可以切换 `Active`
- 在 `DONE` 或 `IDLE` 状态下，也都可以重新勘线

#### 科目1运行中如何强制停止

如果科目1正在跑，比如中途摔倒、跑偏、或者你判断这趟已经没必要继续，直接：

1. 保持当前 `Active=1`
2. 长按一次 `KEY3`

当前代码会立即执行 `subject1_stop()`，也就是：

- 停止当前导航
- 电机目标转速清零
- 电机输出关闭
- 平衡参考清零
- 科目1状态切回 `SUBJ1_STATE_IDLE`

所以对科目1来说，`KEY3` 的语义是统一的：

- 未运行时，`KEY3` 长按 = 启动
- 运行中时，`KEY3` 长按 = 强制停止
- 自己完整跑完时，不需要你再额外长按一次 `KEY3`
...
#### 如果当前 GPS 路点不满意，如何重新定位

现在已经支持在同一上电周期内直接重勘，不需要先重启。

前提条件：

- 当前活跃科目切到 `Active=1`
- 科目1当前不在运行，也就是状态处于 `IDLE` 或 `DONE`
- GPS 已稳定，否则新的打点不会成功

具体操作：

1. 如果车还在跑，先 `KEY3` 长按一次，让科目1停下来。
2. 把车推回新的发车区位置。
3. `KEY4` 长按第一次。
4. 这一次会直接进入“重勘模式”：旧的 RAM 路线会被清掉，并把当前位置记录成新的 `ref_start`。
5. 再把车推到新的掉头区位置。
6. `KEY4` 长按第二次。
7. 这一次会记录新的掉头路点，并自动保存到 Flash，覆盖旧的科目1勘线结果。
8. 然后把车推回发车区，`KEY3` 长按即可按新路线起跑。

重勘时典型串口输出会是：

```text
[SUBJ1] Re-survey started: old route cleared, recording new ref_start.
[SUBJ1] Survey 1/2: ref_start recorded. Move to turn-around point.
[SUBJ1] Survey 2/2: waypoint recorded. Config saved to flash. Ready!
```
...
- 第一次重勘长按 `KEY4` 后，旧路线会先从当前 RAM 工作区清掉，然后开始记录新的起点
- 只有第二次重勘成功后，新的路线才会真正保存到 Flash
- 所以如果你第一步重勘后又后悔了、第二步没完成，那么本次上电期间旧路线不会再保留在 RAM 里
- 但因为 Flash 还没被新路线覆盖，只要断电重启，系统仍会从 Flash 恢复上一次完整保存的旧路线
...
- 若执行重新勘线，则新的第 2 次 `KEY4` 成功后会覆盖保存新的科目1路线
- 下次上电后，如果 Flash 中已恢复出 `ref_start + waypoint`，`subject1_app_init()` 会自动判断勘线已完成
- 因此不是每次上电都必须重新勘线
- 同一上电周期内即使完整跑完一趟科目1，下一次启动前也会自动恢复原始勘线路线，不会再因为回程覆盖工作路点而只能“导航回起点”
...
1. 在 IPS `HOME` 页把 `Active` 调到 `2` → 串口 `Active subject: 2`
2. 将车放在发车区，面朝八字中心
3. `KEY3 长按` → 启动
...
在 IPS `HOME` 页把 `Active` 调到 `3`，串口会打印 `[KEY] Active subject: 3`。切换后 `Ctrl` 会自动同步到该科目对应的舵机控制模式，`KEY3/KEY4` 长按行为也会跟随当前科目。
...
**第一步：切换到科目3模式**
- 在 IPS `HOME` 页把 `Active` 改为 `3`
- 切换后会自动把 `Ctrl` 同步到该科目对应的舵机控制模式
...
| I7 | 科目1回程破坏原始勘线路线 | `subject1_app.c` | 新增科目1私有路线备份；启动前、停止后和完成后自动恢复 `nav_app` 原始去程路点，支持同一上电周期重复起跑 |
| I8 | 科目选择与舵机模式容易错配 | `key_app.c` / `ips_app.c` / `ips_app_config.h` / `cpu0_main.c` | `HOME` 页 `Active` 改为普通可调参数，范围 `0~3`；`Ctrl` 随 `Active` 自动同步，避免科目和模式手动双选错配 |
